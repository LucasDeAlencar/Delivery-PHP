...
window.CarrinhoMenu = {
    ...
    init() {
        ...
        // this.criarElementoCarrinho(); // Popup desabilitado
        this.calcularTaxaEntrega();
        
        // Sincronizar com sistema Carrinho apenas quando necessário
        // setInterval(() => {
        //     this.carregarCarrinho();
        // }, 2000);
    ...
    adicionar(produto) {
        ...
        // Remover cálculo de taxa que está causando erro
        // this.calcularTaxaEntrega();
    },
    ...
    remover(index) {
        if (index >= 0 && index < this.itens.length) {
            ...
            // Salvar diretamente sem removeItem
            localStorage.setItem('carrinho', JSON.stringify(this.itens));
    ...
    criarElementoCarrinho() {
        ...
        // Estrutura alterada para incluir a Sidebar
        const carrinhoHtml = `
                ...
                <div class="carrinho-sidebar">
                    ...
                    <div class="carrinho-body">
                        ...
                        <div class="opcoes-entrega" style="padding: 15px; border-top: 1px solid #333; margin-top: 10px;">
                            ...
                            <div class="row">
                                <div class="col-6">
                                    <button class="btn btn-warning btn-sm w-100 btn-retirada" style="background: #f8b531; border-color: #f8b531; color: #000; font-size: 0.8rem; padding: 8px 4px;">
                                        <i class="fas fa-store mr-1"></i><span class="d-inline d-sm-none">Retirada</span><span class="d-none d-sm-inline">Retirar na Loja</span>
                                    </button>
                                </div>
                                <div class="col-6">
        ...
        // this.configurarEventos(); // Event listeners do popup desabilitados
    },
    ...
    mostrar() {
        // Popup desabilitado - função removida
        return;
    },
    ...
    atualizarConteudo() {
        ...
        // Se o elemento não existe, não fazer nada (carrinho não está visível)
        ...
        this.itens.forEach((item, index) => {
            ...
            if (item.extras && item.extras.length > 0) {
                const extrasNomes = item.extras.map(e => `${e.nome} (${e.quantidade}x)`).join(', ');
                const extrasValor = item.extras.reduce((total, e) => total + (parseFloat(e.preco) * parseInt(e.quantidade)), 0);
                extrasTexto = `
                    <div style="color: #4CAF50; font-size: 11px; margin-top:2px;">
                        <i class="fas fa-plus-circle"></i> ${extrasNomes}
                        <span style="color: #888;"> (+R$ ${extrasValor.toFixed(2).replace('.', ',')})</span>
                    </div>
                `;
            }
